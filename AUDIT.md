...
>
> **Session debug console __hg 2026-05-01** : Ajout de `window.__hg` dans `render.js` (~L381) — objet de debug console pour tester les 4 animations récentes sans attendre leurs conditions de jeu. (1) `__hg.etirement()` : reset `_etirementLastDay` + déclenche séquence t1→t2→t3 (guard stade adult). (2) `__hg.baillement()` : appelle directement `triggerExpr('baillement', 18)` (guard anti-egg). (3) `__hg.frisson()` : reset `_frissonCooldown` + `animator.trigger('frisson')`. (4) `__hg.hochement()` : `animator.trigger('hochement')` (guard teen/adult). (5) `__hg.saut()` : alias `triggerGotchiBounce()` pour comparer. (6) `__hg.etat()` : snapshot complet — stade, heure, météo, animator.active, _expr, _idleFrames, _frissonCooldown, _etirementLastDay + évaluation des conditions de déclenchement automatique. Aucune donnée game mutée (sauf reset des cooldowns de debug). `node --check` : ✅ syntaxe OK.
>
