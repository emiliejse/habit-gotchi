...
> **Session célébration bras ha=5 2026-05-01** : Comportement des bras au niveau bonheur max (ha=5) refondu pour adult et teen. Avant : `bras-leves` s'affichait en permanence via son `when`, le scheduler de poses idle était court-circuité (condition `ha <= HA_ARMS_UP`). Après : le scheduler tourne à tous les niveaux de bonheur ; au niveau 5, il pioche les 3 nouvelles poses célébration (bras gauche levé, bras droit levé, les deux levés) avec 50% de probabilité combinée, les poses normales continuent à 50%. (1) `render-sprites.js` : calque `bras-leves` supprimé et remplacé par 3 calques `bras-g-leve`, `bras-d-leve`, `bras-2-leves` (adult, `when:false` → uniquement via animator) ; 3 calques équivalents `teen-bras-g-leve`, `teen-bras-d-leve`, `teen-bras-2-leves` ajoutés dans `LAYERS_TEEN` ; `bras-normal` adult : condition `ha <= HA_ARMS_UP` retirée (actif à tous niveaux, sert de fallback entre poses) ; `drawAdult()` : condition `ha <= HA_ARMS_UP` retirée du `if (en >= EN_WARN)`, `poseIds` étendu aux 3 nouvelles poses, tirage au sort bifurqué `ha > HA_ARMS_UP` vs normal ; `drawTeen()` : scheduler `_teenPose` ajouté (même mécanique que `_adultPose`, poses célébration uniquement). (2) `render.js` : `window._teenPose = { cooldown: 240 }` initialisé à côté de `_adultPose` ; 6 nouvelles ANIM_DEFs ajoutées (`pose_bras_g_leve`, `pose_bras_d_leve`, `pose_bras_2_leves` pour adult + 3 équivalents `teen_*` pour teen) ; `etirement_t1` mis à jour : `bras-leves` → `bras-2-leves`. (3) `LAYERS_TEEN` : count passe de 20 à 23 calques. `LAYERS_ADULT` : `bras-leves` remplacé par 3 calques, count passe de 27 à 29.
>
