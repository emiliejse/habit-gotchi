...
>
> **Session fix désynchronisation props/corps 2026-04-30** : Bug 2 de `AUDIT_PIXEL_ART.md` résolu dans `render-sprites.js:drawAccessoires` (L395–431) — `accX` et `accY` étaient calculés avec `pxFree()` (floor au pixel) alors que le corps passe par `px()` (floor sur grille PX, paliers de 5px). Résultat : les accessoires "rampaient" pixel par pixel pendant que le corps "sautait" de 5 en 5px → désynchronisation visible à la marche (`walkX`) et au rebond (`bobY`). Fix : `accXraw`/`accYraw` calculés d'abord, puis snapés sur grille PX via `Math.floor(raw / PX) * PX`. Le commentaire erroné justifiant l'absence de snap (L395-397 et L415-417) remplacé par la documentation correcte du fix.
