...
| ✅ ~~🟠 P1~~ | Cohérence bulles ↔ API | ~~Les bulles utilisent `{{diminutif}}` mais les prompts API utilisent `{{userName}}`~~ **CORRIGÉ 2026-05-01** — `{{userName}}` remplacé par `{{diminutif}}` dans `prompts/ai_contexts.json` (templates `askClaude.base` × 3, `genSoutien`, `genBilanSemaine`) et `prompts/ai_system.json` (template `soutien`). `sendSoutienMsg()` (`ui-ai.js:914`) remplace maintenant `{{diminutif}}` au lieu de `{{userName}}`. `{{userName}}` reste dans `vars` mais n'est plus injecté dans les templates voix. | `prompts/ai_contexts.json`, `prompts/ai_system.json`, `js/ui-ai.js:914` |
...
| ✅ ~~🟡 P2~~ | Bulle nuit fallback | ~~`app.js:1056` redéfinit un pool nuit en dur si `src.nuit` est absent — duplique l'info avec `MSG.nuit` (`app.js:160-167`)~~ **CORRIGÉ 2026-05-01** — Pool littéral inline remplacé par `MSG.nuit` (`const pool = src.nuit \|\| MSG.nuit`) — source de vérité unifiée. | `js/app.js:1646` |
